// ==UserScript==
// @name         Twitch UI Cleaner (NO ADs)
// @namespace    http://tampermonkey.net/
// @version      5.4
// @description  A minimal Twitch cleaner that works across all Twitch pages.
// @author       salaminha
//               Carousel removal based on work by LiquidJesus
//               Keep Tab Active based on work by Vikindor (https://vikindor.github.io/)
//               Ad blocking based on TwitchAdSolutions by ryanbr (maintained fork) and cleanlock contributors
// @license      MIT
...
// @downloadURL https://update.greasyfork.org/scripts/572852/Twitch%20UI%20Cleaner%20%28NO%20ADs%29.user.js
// @updateURL https://update.greasyfork.org/scripts/572852/Twitch%20UI%20Cleaner%20%28NO%20ADs%29.meta.js
// ==/UserScript==
...
(function () {
    ...
    // ============================================================
    // TOGGLE FEATURES - Set to true to enable, false to disable
    // ============================================================
    const config = {
        // Remove the featured carousel (stops audio, removes gap)
        removeCarousel: true,

        // Hide top nav promo buttons (Bits, Prime, etc.)
        hidePromoButtons: true,

        // Hide "Using this Extension" banner on channel pages
        hideExtensionBanner: true,

        // Hide stories section in sidebar
        hideStories: true,

        // Block Twitch video ads (uses TwitchAdSolutions vaft v68.3.0 from ryanbr fork)
        blockAds: true,

        // Prevent Twitch from auto-pausing or throttling video when the tab is inactive
        keepTabActive: true
    };
    // ============================================================
    ...
    // ============================================================
    // AD BLOCKER (TwitchAdSolutions vaft v68.3.0)
    // Only runs when config.blockAds is true
    // ============================================================
if (config.blockAds) { (function() {
        ...
        document.querySelectorAll('[class*="carousel"]').forEach(el => {
            if (el.querySelector('video')) {
                el.querySelectorAll('video').forEach(v => {
                    ...
                    v.volume = 0;
                    v.pause();
                    // Remove the src so audio can't restart
                    v.src = '';
                    v.load();
                });
    ...
    function getPromoButtonsCSS() {
        if (!config.hidePromoButtons) return '';
        return `
            div:has(> div > button[data-a-target="top-nav-get-bits-button"]),
            .top-nav__prime,
            button:has(path[d^="m13 8-5.349"])              { display: none !important; }
        `;
    }

    function getExtensionBannerCSS() {
        if (!config.hideExtensionBanner) return '';
        return `
            [data-test-selector="extension-disclaimer"]     { display: none !important; }
        `;
    }

    function getStoriesCSS() {
        if (!config.hideStories) return '';
        return `
            .storiesLeftNavSection--csO9S button > div       { display: none !important; }
            .storiesLeftNavSectionCollapsedButton--txKvw      { display: none !important; }
    ...
    function injectCSS() {
        const style = document.createElement('style');
        style.textContent = `
            /* Top nav promo buttons */
            ${getPromoButtonsCSS()}

            /* Extension banner */
            ${getExtensionBannerCSS()}
    ...
    // Defer carousel observer until document.body exists — at @run-at document-start,
    // document.body is null and observe() would throw, killing the IIFE.
    const startCarouselObserver = () => {
        if (!document.body) return;
        const carouselObserver = new MutationObserver(() => {
            if (config.removeCarousel) nukeCarousel();
        });
    ...
    if (document.body) {
        ...
        document.addEventListener('DOMContentLoaded', startCarouselObserver, { once: true });
    }


    // ============================================================
    // KEEP TAB ACTIVE (based on Twitch - Keep Tab Active by Vikindor)
    // Prevents Twitch from auto-pausing or throttling video when
    // the tab is inactive or loses focus.
    // Only runs when config.keepTabActive is true.
    // ============================================================
    if (config.keepTabActive) {
        ...
        // Override document visibility properties so Twitch always
        // thinks the tab is visible and focused
        ...
        // Silently suppress visibility/freeze/blur events so Twitch
        // never detects the tab going to the background
        ...
        // Guard HTMLMediaElement.pause() — only allow it if triggered
        // within ~1200ms of a real user gesture (click, keydown, etc.)
        ...
        // Resume any video that gets added to the DOM while paused
        ...
        // Proxy IntersectionObserver so player container always appears
        // fully on-screen (prevents Twitch from throttling off-screen video)
        ...
        // Dispatch a synthetic mousemove every 30s to prevent idle timeouts
        uw.setInterval(() => {
            try {
        ...
        // Request a screen wake lock to prevent the OS from sleeping
        try { uw.navigator.wakeLock?.request?.('screen').catch(()=>{}); } catch {}

        // Auto-click "Start Watching" age-gate overlay if it appears
        ...
        new uw.MutationObserver(tryClickStartWatching)
            .observe(uw.document.documentElement, {childList: true, subtree: true, attributes: true});

        // Auto-click any content-gate overlay button (e.g. mature content warnings)
